...
public class MeetingCalendar {
	...
	public CalendarBean createCalendar(int... args) {
		...
		// カレンダーに年を設定
		calenderBean.setYear(calendar.get(Calendar.YEAR));
		// カレンダーに月を設定
		calenderBean.setMonth(calendar.get(Calendar.MONTH) + 1);
		...
		for (int i = 0; i < rows; i++) {
			for (int j = 0; j < 7; j++) {
				if (i == 0 && j < before || i == rows - 1 && j >= (7 - after)) {
					...
					if (now.get(Calendar.DATE) == date && now.get(Calendar.MONTH) == calenderBean.getMonth() - 1
							&& now.get(Calendar.YEAR) == calenderBean.getYear()) {
						data[i][j] = "*" + data[i][j];
					}
				}
			}
